...
@Component({
  selector: 'app-obregon',
  templateUrl: './obregon.component.html',
  styleUrls: ['./obregon.component.css']
})
export class ObregonComponent implements OnInit {
  lat: number = 27.482773;
  lon: number = -109.930367;
  ...
  getChartCelsius() {
    ...
    console.log('ChartCelsius');

    console.log('Current: ' + formatDate(this.current_date, 'yyyy-MM-dd', 'en'));

    console.log('Yesterday: ' + formatDate(this.current_date.setDate(this.current_date.getDate() - 1), 'yyyy-MM-dd', 'en'));

    this.http.get('https://api.weatherbit.io/v2.0/history/daily?'
      + this.getLat() + '&' + this.getLon() + '&start_date=' + formatDate(this.dateMinus2.setDate(this.dateMinus2.getDate() - 1), 'yyyy-MM-dd', 'en')
      + '&end_date=' + formatDate(this.dateMinus1.setDate(this.dateMinus1.getDate()), 'yyyy-MM-dd', 'en')
      + '&key=4c2236b4850643d29d0a3c7f6979be3e')
      .subscribe((response) => {
        this.weather = response;
        console.log(this.weather);
        let temp_max = this.weather.map(response => response.data.max_temp);
        console.log(this.weather.data[0].min_temp);
        let temp_min = this.weather.map(response => response.data[0].min_temp);
    ...
    formatDate(this.current_date, 'yyyy-MM-dd', 'en')
  ...
  getChartFarenheit() {
    ...
    console.log(formatDate(this.current_date, 'yyyy-MM-dd', 'en'));

    console.log(formatDate(this.current_date.setDate(this.current_date.getDate() - 1), 'yyyy-MM-dd', 'en'));
    this.http.get('https://api.weatherbit.io/v2.0/current?'
      + this.getLat() + '&' + this.getLon() + '&start_date=' + formatDate(this.current_date, 'yyyy-MM-dd', 'en')
      + '&end_date=' + formatDate(this.current_date.setDate(this.current_date.getDate() - 1), 'yyyy-MM-dd', 'en')
      + '&units=I' + '&key=4c2236b4850643d29d0a3c7f6979be3e')
      .subscribe((response) => {
        this.weather = response;
